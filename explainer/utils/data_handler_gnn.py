...
class DataHandler:
    ...
    def load_data(self):
        ...
        trn_loader = DataLoader(trn_dataset, batch_size=args_gnn.batch_size, shuffle=True)

        # load validation batch
        user_list, item_list = self.load_csv(self.val_path)
        val_dataset = Dataset(user_list, item_list)
        val_loader = DataLoader(val_dataset, batch_size=args_gnn.batch_size, shuffle=True)

        # load testing batch
        user_list, item_list = self.load_csv(self.tst_path)
        tst_dataset = Dataset(user_list, item_list)
        tst_loader = DataLoader(tst_dataset, batch_size=args_gnn.batch_size, shuffle=True)
